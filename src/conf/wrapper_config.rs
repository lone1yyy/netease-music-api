...
impl WrapperConfig {
    pub fn default() -> super::Result<WrapperConfig> {
        WrapperConfig::read_config()
    }

    fn read_config() -> super::Result<WrapperConfig> {
        let file = WrapperConfig::find()?;
        ...
        WrapperConfig::parse(contents, &file)
    }
    ...
    fn get(&self, env: Environment) -> &BasicConfig {
    ...
    fn active_default_from(filename: Option<&Path>) -> super::Result<WrapperConfig> {
    ...
    pub fn active_default() -> super::Result<BasicConfig> {
        Ok(BasicConfig::new(Environment::active()?))
    }

    pub fn active(&self) -> &BasicConfig {
        self.get(self.active_env)
    }

    fn parse<P: AsRef<Path>>(src: String, filename: P) -> super::Result<WrapperConfig> {
        ...
        let mut config = WrapperConfig::active_default_from(Some(filename.as_ref()))?;
        ...
        for (entry, value) in table {
            ...
            let env = entry.as_str().parse::<Environment>().unwrap();
            let conf = config.get_mut(env);
            ...
            let port = kv_pairs.get("port").unwrap().as_str().unwrap();
